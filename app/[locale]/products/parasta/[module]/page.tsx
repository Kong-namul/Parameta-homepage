...
import { getParastaModules, parastaModules, type ParaStaModuleKey } from "@/lib/parasta-modules";
...
const COPY = {
  ...
  en: {
    ...
    talkToSales: "Talk to sales",
    whatItDoes: "WHAT IT DOES",
    whatItDoesTitle: "Core capabilities.",
    architecture: "ARCHITECTURE",
    architectureTitle: "System architecture.",
    architecturePlaceholder: "Interactive diagram placeholder — populated with SVG during Phase 11 content.",
    bm: "BM MODEL",
    bmTitle: "Business model.",
    bmDesc: "Transaction fees plus subscription. Per-module pricing is shared at the quote stage.",
    closingPrefix: "Ready to integrate",
    contact: "Contact",
  },
};
...
export default async function ModulePage({ params }: { params: Params }) {
  const { locale, module: moduleKey } = await params;
  const modules = getParastaModules(locale);
  const m = modules[moduleKey as ParaStaModuleKey];
