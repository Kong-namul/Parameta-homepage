...
export default async function NewsroomList() {
  const c = COPY;
  let items: any[] = [];
  try {
        ...
        <Container>
          {items.length === 0 ? (
            ...
            <ul className="space-y-4">
              {items.map((n: any) => (
                <li key={n._id}>
                  <Link href={n.external_link || `/company/newsroom/${n._id}`} target={n.external_link ? "_blank" : undefined} rel={n.external_link ? "noopener noreferrer" : undefined}>
                    <Card variant="elevated">
                      <div className="flex justify-between items-start gap-4">
                        <h3 className="font-semibold">{n.title_ko}</h3>
